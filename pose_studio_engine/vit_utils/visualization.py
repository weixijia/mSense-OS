...
def draw_points(image, points, color_palette='tab20', palette_samples=16, confidence_threshold=0.5, mono_color=None):
    """
    ...
    Args:
        ...
        confidence_threshold: only points with a confidence higher than this threshold will be drawn. Range: [0, 1]
            Default: 0.5
        mono_color: if not None, all points will be drawn with this color (BGR format)
            Default: None
    ...
    for i, pt in enumerate(points):
        if pt[2] > confidence_threshold:
            image = cv2.circle(image, (int(pt[1]), int(pt[0])), circle_size, tuple(colors[i % len(colors)] if mono_color is None else colors[0]), -1)

    return image


def draw_skeleton(image, points, skeleton, color_palette='Set2', palette_samples=8, person_index=0,
                 confidence_threshold=0.5, mono_color=None, skeleton_thickness=2):
    """
    ...
    Args:
        ...
        confidence_threshold: only points with a confidence higher than this threshold will be drawn. Range: [0, 1]
            Default: 0.5
        mono_color: if not None, all skeleton lines will be drawn with this color (BGR format)
            Default: None
    ...
    for i, joint in enumerate(skeleton):
        ...
        if pt1[2] > confidence_threshold and pt2[2] > confidence_threshold:
            image = cv2.line(
                ...
                tuple(colors[0] if mono_color is not None else colors[person_index % len(colors)]), 
                skeleton_thickness
            )
...
def draw_points_and_skeleton(image, points, skeleton, points_color_palette='tab20', points_palette_samples=16,
    ...
    Args:
        ...
        confidence_threshold: only points with a confidence higher than this threshold will be drawn. Range: [0, 1]
            Default: 0.5
        mono_color: if not None, all points and skeleton lines will be drawn with this color (BGR format)
            Default: None
        skeleton_thickness: thickness of skeleton lines
            Default: 2
    ...
    image = draw_skeleton(image, points, skeleton, color_palette=skeleton_color_palette,
                          ...
                          confidence_threshold=confidence_threshold, mono_color=mono_color,
