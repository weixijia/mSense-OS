...
> 7. **mmWave radar (TI AWR1843 + DCA1000):** read **[`mmwave_pure_python/SETUP.md`](mmwave_pure_python/SETUP.md)** §0 first (network: static IP `192.168.33.30`; UDP `rmem` tuning). **Current capture workflow (2026-06-22):** bring the radar up once in **mmWave Studio (Windows)** with `StartFrame` numFrames=0 (infinite) so radar + DCA1000 stream autonomously, **reboot the host to Ubuntu without power-cycling the radar**, then capture receive-only with **`python main.py --no-camera --no-trigger`**. Frame loss is eliminated by an off-GIL C receiver (`core/mmwave_capture_c.py`; fpga_udp patch in `mmwave_pure_python/patches/`) — 11.4 fps under load, zero kernel packet loss, complete-frames-only (no interpolation). RD orientation matches the trained model via `config.MMWAVE_RD_FLIP_RANGE = True`. (The pure-Python `python main.py --trigger` path is parked — see SETUP.md.)
